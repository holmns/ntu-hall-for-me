...
export function ListingCard({
  ...
  const head = (
    <div className="min-w-0">
      <div className="flex items-start justify-between gap-4">
        <h3 className="text-[15px] font-semibold leading-snug text-ink group-hover:text-brand-ink">
          {listing.title}
          {unavailable && (
  ...
  const rankChip = rank != null && (
    ...
    <span
      data-rank
      className="grid h-6 w-6 place-items-center rounded-full bg-brand-soft text-xs font-semibold text-brand-ink tabular-nums shadow-[0_1px_3px_rgba(28,26,23,0.14)]"
    >
      {rank}
    </span>
  );
